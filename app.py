...
if user is None:
    ...
    with col2:
        ...
        if demo_mode():
            st.warning("Demo mode is enabled. Do not use confidential information.")
            with st.expander("View demo accounts"):
                for demo_email, info in USERS.items():
                    st.markdown(f"- **{demo_email}** — {info['name']} · _{get_role_label(info['role'])}_")
