...
render_navbar("m2")
require_access("m2")
render_breadcrumb("AI Strategy & Use-Case Portfolio", "Use-case Scoring & Prioritization")
render_subtabs(
    [("📊 Use-case Scoring & Prioritization", "pages/2_Feasibility_Assessment.py"),
     ("⚖️ Value-Risk Assessment", "pages/3_Gain_Pain_Analysis.py")],
...
st.title("Use-case Scoring & Prioritization")
...
if problem_id not in cache:
    ...
    if not result:
        st.error("Assessment failed. Please check your AI Settings (provider, model, API key) and try again.")
        st.stop()
...
with rec_col:
    ...
    if result.get("recommendations"):
        st.markdown("**💡 Recommendations**")
        st.caption("AI recommendation and human review are retained together.")
        h1, h2 = st.columns([1.35, 1])
        h1.markdown("**AI Recommendation**")
        h2.markdown("**Feedback / Comment**")
        for i, r in enumerate(result["recommendations"]):
            ai_col, human_col = st.columns([1.35, 1], vertical_alignment="top")
            ai_col.markdown(f"{i + 1}. {r}")
            with human_col:
                render_human_comment(problem_id, "feasibility", i, label="Feedback / Comment")
with risk_col:
    if result.get("risks"):
