...
st.set_page_config(
    page_title="AI Delivery & Operational Readiness - Assumptions",
    page_icon="🧪",
    layout="wide",
)
...
render_breadcrumb(
    "AI Delivery & Operational Readiness",
...
if step == 1:
    ...
    if hypotheses:
        ...
        for i, h in enumerate(hypotheses):
            ...
            with st.container(border=True):
                ...
                proposal_col, human_col = st.columns([1.35, 1], vertical_alignment="top")
                proposal_col.caption(
                    "AI-proposed hypothesis. Record agreement, context, conditions, or an override before approval."
                )
                with human_col:
    ...
    for i, exp in enumerate(experiments):
        ...
        with st.expander(
            ...
            proposal_col, human_col = st.columns([1.35, 1], vertical_alignment="top")
            proposal_col.caption(
                "AI-generated experiment recommendation for this hypothesis."
            )
            with human_col:
                render_human_comment(
                    project["id"],
    ...
    try:
        questions = load_feedback_questions(project["id"])
    except Exception:
        # Recover from any malformed questionnaire saved by an older build.
        questions = None

    if not isinstance(questions, dict) or not questions.get("stakeholders"):
        questions = None
    ...
    if questions is None:
        ...
        with st.spinner(
            ...
            try:
                ...
                questions = fallback_feedback_questions(project)
    ...
    **Objective**
    ...
    if summary:
        ...
        for title, value in rows:
            st.markdown(f"### {title}")
            st.write(value)
            if title == "MVP Recommendation":
                render_human_comment(
                    project["id"],
                    "stakeholder_mvp_recommendation",
                    current_role,
                    label="Human Comment",
                )
