...
st.markdown(
    ...
    [data-testid="stSegmentedControl"] button,[data-testid="stButtonGroup"] button{min-height:38px!important;min-width:108px!important;padding:5px 7px!important;white-space:nowrap!important;background:#fff!important;color:#405678!important;border-color:#d9e2f0!important;font-weight:750!important;flex:1 0 108px!important;overflow:visible!important}
...
def stepper(active, completed):
    ...
    for index, (col, label) in enumerate(zip(cols, STAGES), 1):
        ...
        icon = "✓  " if index in completed else ""
        with col:
...
if step == 1:
    ...
    with st.container(border=True):
        ...
        heads = st.columns([1.45, 2.25, 2.4, 2.3, 1.75])
        ...
        with st.form(f"failure_{project_id}"):
            for i, (scenario, impact, backup) in enumerate(content["failures"]):
                c = st.columns([1.45, 2.25, 2.4, 2.3, 1.75], vertical_alignment="center")
