...
st.set_page_config(page_title="AI Delivery & Operational Readiness - Data", page_icon="🗄️", layout="wide")
require_login()
apply_theme()
render_sidebar(active="pe_data")
render_navbar(active="pe_data")
require_access("pe_data")
render_breadcrumb("AI Delivery & Operational Readiness", "Data")
...
def _render_structured(project_id, requirements, review):
    ...
    with st.container(border=True):
        ...
        for group_name, category_name, attribute in attributes:
            ...
            row = st.columns([3.6, 1, 2.4, 3], vertical_alignment="center")
            ...
            human_comment = row[3].text_input(
                "Feedback / Comment",
                value=saved.get("human_comment", ""),
                placeholder="Add context or an override…",
                label_visibility="collapsed",
                key=f"comment_{_key(project_id, attr_id)}",
            )
            attribute_review[attr_id] = {
                "available": available,
...
def _render_unstructured(project_id, requirements, review):
    ...
    with st.container(border=True):
        ...
        header = st.columns([3.2, 2.2, 2, 2.6])
        ...
        for group_name, category_name, source in sources:
            ...
            if isinstance(saved, dict):
                saved_trust = saved.get("classification", "— Select —")
                saved_comment = saved.get("human_comment", "")
            else:
                saved_trust = saved
                saved_comment = ""
            row = st.columns([3.2, 2.2, 2, 2.6], vertical_alignment="center")
            row[0].markdown(f"**{source}**")
            row[1].markdown(category_name)
            trust = row[2].selectbox("Trustworthiness", TRUST_OPTIONS, index=TRUST_OPTIONS.index(saved_trust) if saved_trust in TRUST_OPTIONS else 0, label_visibility="collapsed", key=f"trust_{_key(project_id, source_id)}")
            human_comment = row[3].text_input(
                "Feedback / Comment",
                value=saved_comment,
                placeholder="Add context or an override…",
                label_visibility="collapsed",
                key=f"source_comment_{_key(project_id, source_id)}",
            )
            trusts[source_id] = {
