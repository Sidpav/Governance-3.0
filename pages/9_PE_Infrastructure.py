...
from ui.human_comment import render_human_comment


st.set_page_config(page_title="AI Delivery & Operational Readiness - Infrastructure", page_icon="🏗️", layout="wide")
require_login()
apply_theme()
render_sidebar(active="pe_infrastructure")
render_navbar(active="pe_infrastructure")
require_access("pe_infrastructure")
render_breadcrumb("AI Delivery & Operational Readiness", "Infrastructure")
...
def action_row(back_step=None, next_step=None, next_label="Next"):
    ...
    with left:
        if back_step and st.button("← Back", key=f"back_{back_step}_{next_step}", use_container_width=True):
            ...
            st.rerun()
    with right:
        if next_step and st.button(next_label, key=f"next_{back_step}_{next_step}", type="primary", use_container_width=True):
...
if step == 1:
    with st.container(border=True):
        ...
        with st.form("infrastructure_intake"):
            ...
            submitted = submit_col.form_submit_button("Generate AI Recommendation →", type="primary", use_container_width=True)
    ...
    with st.container(border=True):
        ...
        heads = st.columns([1.35, 2.75, 3.25, 2.25])
        for col, label in zip(heads, ["Decision Area", "AI Recommendation", "Reasoning", "Feedback / Comment"]):
            col.markdown(f'<div class="infra-table-head">{label}</div>', unsafe_allow_html=True)
        for index, row in enumerate(recommendation.get("recommendations", [])):
            cols = st.columns([1.35, 2.75, 3.25, 2.25], vertical_alignment="top")
    ...
    with st.container(border=True):
        ...
        with st.form("security_review"):
            ...
            next_security = right.form_submit_button("Generate Infrastructure Output →", type="primary", use_container_width=True)
        if back:
            st.session_state.infra_step = 2
            st.session_state[project_step_key] = 2
            st.rerun()
    ...
    with st.container(border=True):
        ...
        edited = {}
        edited_comments = {}
        current_output_comments = saved_output.get("output_comments", {})
        for label, key in field_map:
            lcol, fcol, ccol = st.columns([1.8, 5.3, 2.9], vertical_alignment="center")
            lcol.markdown(f'<div class="output-label">{label}</div>', unsafe_allow_html=True)
            edited[key] = fcol.text_input(label, value=str(final_output.get(key, "")), key=f"infra_output_{project_id}_{key}", label_visibility="collapsed")
            edited_comments[key] = ccol.text_input(
                f"Feedback / comment for {label}", value=current_output_comments.get(key, ""),
                key=f"infra_output_comment_{project_id}_{key}", placeholder="Feedback / comment…",
        ...
        decision = fcol.segmented_control(
            "Approval Status", options,
            default=current_decision if current_decision in options else options[-1],
            key=f"infra_approval_status_{project_id}", label_visibility="collapsed",
        ) or current_decision
        comments = st.text_area(
            "Reviewer Comments", value=saved_output.get("comments", ""),
            key=f"infra_reviewer_comments_{project_id}",
            placeholder="Record conditions, owners, and follow-up actions…",
        )
        st.markdown('<span class="infra-save"></span>', unsafe_allow_html=True)
        saved = st.button(
            "✓  Save Infrastructure Output", type="primary",
            key=f"save_infrastructure_output_{project_id}", use_container_width=False,
        )
        if saved:
            saved_output["final_output"] = edited
            saved_output["output_comments"] = edited_comments
    ...
    if right.button("Proceed → Operational Integration & Metrics", type="primary", use_container_width=True):
