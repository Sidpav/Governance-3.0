...
if step == 1:
    ...
    with st.container(border=True):
        ...
        for col, label in zip(heads, ["Decision Area", "AI Recommendation", "Reasoning", "Human Comment"]):
        ...
        with st.form("infrastructure_output"):
            ...
            for label, key in field_map:
                ...
                edited_comments[key] = ccol.text_input(
                    f"Human comment for {label}",
                    value=current_output_comments.get(key, ""),
                    placeholder="Human comment…",
            ...
            saved = st.form_submit_button("✓  Save Infrastructure Output", type="primary", use_container_width=False)
        if saved:
