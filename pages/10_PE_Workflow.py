...
st.markdown(
    ...
    [data-testid="stSegmentedControl"] button,[data-testid="stButtonGroup"] button{min-height:38px!important;min-width:0!important;white-space:nowrap!important;padding:5px 5px!important;background:#fff!important;color:#405678!important;border-color:#d9e2f0!important;font-weight:750!important;flex:1 1 0!important;overflow:visible!important}
    [data-testid="stSegmentedControl"] button p,[data-testid="stButtonGroup"] button p{font-size:.82rem!important;white-space:nowrap!important;overflow:visible!important;text-overflow:clip!important}
...
if step == 1:
    ...
    with st.form(f"metrics_{project_id}"):
        for title, key in metric_groups:
            with st.container(border=True):
                ...
                h = st.columns([1.55, 1.95, 1.65, 2.0, 1.75])
                ...
                for i, (name, purpose, target, custom) in enumerate(rows):
                    c = st.columns([1.55, 1.95, 1.65, 2.0, 1.75], vertical_alignment="center")
    ...
    with st.container(border=True):
        ...
        h = st.columns([1.35, 3.15, 1.35, 2.0, 1.7])
        ...
        with st.form(f"explain_{project_id}"):
            ...
            for i, (who, need, fmt, custom) in enumerate(rows):
                c = st.columns([1.35, 3.15, 1.35, 2.0, 1.7], vertical_alignment="center")
                c[0].markdown('<span class="wf-row"></span>', unsafe_allow_html=True)
                if custom:
    ...
    with st.container(border=True):
        ...
        h = st.columns([1.05, 1.4, 1.25, 1.55, 2.0, 1.4])
        ...
        with st.form(f"consume_{project_id}"):
            ...
            for i, (who, fmt, access, delivery, custom) in enumerate(rows):
                c = st.columns([1.05, 1.4, 1.25, 1.55, 2.0, 1.4], vertical_alignment="center")
                c[0].markdown('<span class="wf-row"></span>', unsafe_allow_html=True)
                if custom:
