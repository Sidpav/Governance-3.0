...
st.markdown(
    ...
    div[data-testid="stHorizontalBlock"]:has(.wf-marker){width:calc(100% + 4.4rem);margin:0 -2.2rem 30px;padding:7px 25px;background:#fff;border-bottom:1px solid #d9e2f0;gap:8px}
    ...
    div[data-testid="column"]:has(.wf-done) .stButton>button{color:#00a36c!important}
    ...
    div[data-testid="stVerticalBlockBorderWrapper"]{background:#fff;border-color:#d9e2f0!important;padding:10px 12px}
...
STAGES = ["Workflow Placement", "Performance Metrics", "Explainability", "Consumability", "Workflow Output"]
...
def stepper(active, completed):
    ...
    for i, (col, label) in enumerate(zip(cols, STAGES), 1):
        ...
        icon = "✓" if i in completed else str(i)
        with col:
...
if step == 1:
    ...
    with st.container(border=True):
        st.markdown('<div class="wf-card-title">Workflow Placement</div>', unsafe_allow_html=True)
        h = st.columns([1.55, 2.15, 2.8, 2.3, 2.4])
        for col, title in zip(h, ["Workflow Area", "How the Prototype Fits", "Owner", "Approval", "Feedback / Comment"]): col.markdown(f'<div class="wf-head">{title}</div>', unsafe_allow_html=True)
        values = []
        with st.form(f"placement_{project_id}"):
            for i, (area, fit) in enumerate(content["placements"]):
                cols = st.columns([1.55, 2.15, 2.8, 2.3, 2.4], vertical_alignment="center")
                cols[0].markdown('<span class="wf-row"></span>'+f'<div class="wf-name">{area}</div>', unsafe_allow_html=True)
                ...
                status = cols[3].segmented_control("Approval", ["Approved", "Pending", "Rejected"], default=row_value(saved, "placements", i, "status", "Pending"), key=f"pl_status_{project_id}_{i}", label_visibility="collapsed") or "Pending"
                ...
                values.append({"area": area, "fit": fit, "owner": owner, "status": status, "comment": comment})
    ...
    metric_groups = [("Business Metrics", "business"), ("Technical Metrics by Component", "technical"), ("Environmental / Green Metrics", "environmental")]
    with st.form(f"metrics_{project_id}"):
        for title, key in metric_groups:
            with st.container(border=True):
                ...
                labels = ["Metric" if key != "technical" else "Component", "Purpose" if key != "technical" else "Technical Metric", "Target", "Status", "Feedback / Comment"]
                for col, label in zip(h, labels): col.markdown(f'<div class="wf-head">{label}</div>', unsafe_allow_html=True)
                result = []
                for i, (name, purpose, target) in enumerate(content[key]):
                    c = st.columns([1.7, 2.15, 1.8, 1.25, 2.2], vertical_alignment="center")
                    c[0].markdown('<span class="wf-row"></span>'+f'<div class="wf-name">{name}</div>', unsafe_allow_html=True)
                    ...
                    status = c[3].segmented_control("Status", ["Defined", "Pending"], default=row_value(saved, key, i, "status", "Pending"), key=f"{key}_status_{project_id}_{i}", label_visibility="collapsed") or "Pending"
                    ...
                    result.append({"name": name, "purpose": purpose, "target": target, "status": status, "comment": comment})
    ...
    with st.container(border=True):
        st.markdown('<div class="wf-card-title">Explainability by Stakeholder</div>', unsafe_allow_html=True)
        h = st.columns([1.55, 3.6, 1.55, 1.25, 2.1])
        for col, title in zip(h, ["Stakeholder", "Explanation Needed", "Format", "Status", "Feedback / Comment"]): col.markdown(f'<div class="wf-head">{title}</div>', unsafe_allow_html=True)
        values = []
        with st.form(f"explain_{project_id}"):
            for i, (who, need, fmt) in enumerate(content["explainability"]):
                c = st.columns([1.55, 3.6, 1.55, 1.25, 2.1], vertical_alignment="center")
                c[0].markdown('<span class="wf-row"></span>'+f'<div class="wf-name">{who}</div>', unsafe_allow_html=True); c[1].markdown(f'<div class="wf-copy">{need}</div>', unsafe_allow_html=True); c[2].markdown(f'<div class="wf-copy">{fmt}</div>', unsafe_allow_html=True)
                status = c[3].segmented_control("Status", ["Defined", "Pending"], default=row_value(saved, "explainability", i, "status", "Pending"), key=f"ex_status_{project_id}_{i}", label_visibility="collapsed") or "Pending"
                ...
                values.append({"stakeholder": who, "need": need, "format": fmt, "status": status, "comment": comment})
    ...
    with st.container(border=True):
        st.markdown('<div class="wf-card-title">Consumability</div>', unsafe_allow_html=True)
        h = st.columns([1.4, 2.2, 1.8, 2.2, 2.3])
        for col, title in zip(h, ["User", "Format", "Access Level", "Delivery Mechanism", "Feedback / Comment"]): col.markdown(f'<div class="wf-head">{title}</div>', unsafe_allow_html=True)
        values = []
        with st.form(f"consume_{project_id}"):
            for i, (who, fmt, access, delivery) in enumerate(content["consumability"]):
                c = st.columns([1.4, 2.2, 1.8, 2.2, 2.3], vertical_alignment="center")
                c[0].markdown('<span class="wf-row"></span>'+f'<div class="wf-name">{who}</div>', unsafe_allow_html=True); c[1].markdown(f'<div class="wf-copy">{fmt}</div>', unsafe_allow_html=True); c[2].markdown(f'<div class="wf-copy">{access}</div>', unsafe_allow_html=True); c[3].markdown(f'<div class="wf-copy">{delivery}</div>', unsafe_allow_html=True)
                comment = c[4].text_input("Feedback / Comment", value=row_value(saved, "consumability", i, "comment"), key=f"co_comment_{project_id}_{i}", label_visibility="collapsed", placeholder="Add feedback…")
                values.append({"user": who, "format": fmt, "access": access, "delivery": delivery, "comment": comment})
    ...
    requirements = [
        ("Workflow placement approved", bool(saved.get("placements")) and all(x.get("status") == "Approved" for x in saved.get("placements", []))),
        ("Business metrics selected", bool(saved.get("business")) and all(x.get("status") == "Defined" for x in saved.get("business", []))),
        ("Technical metrics selected", bool(saved.get("technical")) and all(x.get("status") == "Defined" for x in saved.get("technical", []))),
        ("Environmental metrics selected", bool(saved.get("environmental")) and all(x.get("status") == "Defined" for x in saved.get("environmental", []))),
        ("Explainability requirements defined", bool(saved.get("explainability")) and all(x.get("status") == "Defined" for x in saved.get("explainability", []))),
        ("Consumability views defined", bool(saved.get("consumability"))),
