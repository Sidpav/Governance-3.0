...
st.markdown(
    ...
    [data-testid="stSegmentedControl"] button,[data-testid="stButtonGroup"] button{min-height:38px!important;min-width:0!important;white-space:nowrap!important;padding:5px 10px!important;background:#fff!important;color:#405678!important;border-color:#d9e2f0!important;font-weight:750!important;flex:1 1 0!important}
...
if step == 1:
    ...
    with st.form(f"metrics_{project_id}"):
        for title, key in metric_groups:
            with st.container(border=True):
                ...
                h = st.columns([1.65, 2.05, 1.75, 1.65, 1.9])
                ...
                for i, (name, purpose, target, custom) in enumerate(rows):
                    c = st.columns([1.65, 2.05, 1.75, 1.65, 1.9], vertical_alignment="center")
    ...
    with st.container(border=True):
        ...
        h = st.columns([1.45, 3.35, 1.45, 1.65, 1.85])
        ...
        with st.form(f"explain_{project_id}"):
            ...
            for i, (who, need, fmt, custom) in enumerate(rows):
                c = st.columns([1.45, 3.35, 1.45, 1.65, 1.85], vertical_alignment="center")
                c[0].markdown('<span class="wf-row"></span>', unsafe_allow_html=True)
                if custom:
    ...
    with st.container(border=True):
        ...
        h = st.columns([1.15, 1.55, 1.35, 1.65, 1.55, 1.55])
        ...
        with st.form(f"consume_{project_id}"):
            ...
            for i, (who, fmt, access, delivery, custom) in enumerate(rows):
                c = st.columns([1.15, 1.55, 1.35, 1.65, 1.55, 1.55], vertical_alignment="center")
                c[0].markdown('<span class="wf-row"></span>', unsafe_allow_html=True)
                if custom:
