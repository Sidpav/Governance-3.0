...
st.markdown(
    ...
    .wf-card-title{font-size:1.15rem;font-weight:800;color:#10203e;margin:2px 0 8px}
    .wf-section{font-size:1.08rem;font-weight:800;color:#10203e;margin:4px 0 7px}
    ...
    .wf-head{color:#526a90;font-size:.94rem;font-weight:800;padding:6px 2px;border-bottom:1px solid #d9e2f0}
    .wf-name{color:#10203e;font-weight:800;font-size:.93rem;padding:4px 2px;margin:0}
    .wf-copy{color:#405678;font-size:.91rem;line-height:1.28;padding:4px 2px;margin:0}
    div[data-testid="stHorizontalBlock"]:has(.wf-row){align-items:center;border-bottom:1px solid #e7edf6;padding:3px 0;min-height:54px;gap:.75rem}
    div[data-testid="stHorizontalBlock"]:has(.wf-row)>div[data-testid="column"]{align-self:center!important}
    div[data-testid="stHorizontalBlock"]:has(.wf-row) [data-testid="stVerticalBlock"]{gap:0!important}
    div[data-testid="stHorizontalBlock"]:has(.wf-row) [data-testid="stElementContainer"]{margin:0!important}
    .wf-row{display:none}
    [data-testid="stSegmentedControl"]>div,
    [data-testid="stSegmentedControl"] [role="radiogroup"],
    [data-testid="stButtonGroup"]>div{display:flex!important;flex-flow:row nowrap!important;width:100%!important;gap:0!important}
    [data-testid="stSegmentedControl"] button,[data-testid="stButtonGroup"] button{min-height:38px!important;min-width:84px!important;white-space:nowrap!important;padding:5px 6px!important;background:#fff!important;color:#405678!important;border-color:#d9e2f0!important;font-weight:750!important;flex:1 0 84px!important;overflow:visible!important}
    [data-testid="stSegmentedControl"] button p,[data-testid="stButtonGroup"] button p{font-size:.78rem!important;white-space:nowrap!important;overflow:visible!important;text-overflow:clip!important;max-width:none!important;width:auto!important}
    ...
    button[data-testid*="segmented_controlActive"]{
        background:#e8f8f1!important;color:#00875a!important;border-color:#00a36c!important;
        box-shadow:inset 0 0 0 1px #00a36c!important
    }
    button[data-testid="stBaseButton-segmented_controlActive"] p,
    button[data-testid*="segmented_controlActive"] p{color:#00875a!important;font-weight:800!important}
    button[data-testid="stBaseButton-segmented_controlActive"] p::before,
    button[data-testid*="segmented_controlActive"] p::before{content:"✓ ";color:#00a36c;font-weight:900}
    [data-testid="stSegmentedControl"] button:focus-visible,
    [data-testid="stButtonGroup"] button:focus-visible{outline:3px solid rgba(21,94,239,.24)!important;outline-offset:2px!important}
    [data-testid="stSegmentedControl"] button p,[data-testid="stButtonGroup"] button p{color:inherit!important}
    div[data-testid="stVerticalBlockBorderWrapper"]{background:#fff;border-color:#d9e2f0!important;padding:2px!important}
    div[data-testid="stVerticalBlockBorderWrapper"]>div{padding:5px 6px!important}
    div[data-testid="stVerticalBlockBorderWrapper"] [data-testid="stTextInput"] input{min-height:40px!important;height:40px!important;padding:6px 10px!important}
    ...
    .wf-output-marker{display:none}
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.wf-output-marker) [data-testid="stCheckbox"]{margin:0!important;padding:2px 0!important}
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.wf-output-marker) [data-testid="stCheckbox"] label{min-height:38px!important;align-items:center!important}
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.wf-output-marker) [data-testid="stCheckbox"] p{font-weight:750!important;color:#10203e!important;font-size:.96rem!important}
...
if step == 1:
    ...
    with st.form(f"metrics_{project_id}"):
        for title, key in metric_groups:
            with st.container(border=True):
                ...
                h = st.columns([1.5, 1.85, 1.55, 2.35, 1.65])
                ...
                for i, (name, purpose, target, custom) in enumerate(rows):
                    c = st.columns([1.5, 1.85, 1.55, 2.35, 1.65], vertical_alignment="center")
    ...
    with st.container(border=True):
        ...
        h = st.columns([1.3, 2.9, 1.3, 2.35, 1.6])
        ...
        with st.form(f"explain_{project_id}"):
            ...
            for i, (who, need, fmt, custom) in enumerate(rows):
                c = st.columns([1.3, 2.9, 1.3, 2.35, 1.6], vertical_alignment="center")
                c[0].markdown('<span class="wf-row"></span>', unsafe_allow_html=True)
                if custom:
    ...
    with st.container(border=True):
        ...
        h = st.columns([1.0, 1.3, 1.2, 1.4, 2.35, 1.3])
        ...
        with st.form(f"consume_{project_id}"):
            ...
            for i, (who, fmt, access, delivery, custom) in enumerate(rows):
                c = st.columns([1.0, 1.3, 1.2, 1.4, 2.35, 1.3], vertical_alignment="center")
                c[0].markdown('<span class="wf-row"></span>', unsafe_allow_html=True)
                if custom:
    ...
    prior_checks = saved.get("output_checklist", {})
    checklist_values = {}
    with st.container(border=True):
        st.markdown('<span class="wf-output-marker"></span>', unsafe_allow_html=True)
        st.markdown('<div class="wf-card-title">Workflow Output</div>', unsafe_allow_html=True)
        for index, (label, derived_ready) in enumerate(requirements):
            checklist_values[label] = st.checkbox(
                label,
                value=bool(prior_checks.get(label, derived_ready)),
                key=f"wf_output_check_{project_id}_{index}",
            )
        st.markdown('<span class="wf-save-marker"></span>', unsafe_allow_html=True)
        if st.button("✓  Save Workflow Output", type="primary", key=f"save_workflow_output_{project_id}"):
            saved["output_checklist"] = checklist_values
            saved["output_complete"] = all(checklist_values.values())
            saved["output_saved"] = True
            saved["saved_by"] = user["email"]
            completed.add(5)
            saved["completed_steps"] = sorted(completed)
            saved["active_step"] = 5
            save_workflow(project_id, saved)
            st.success("Workflow output saved. You can now proceed to Responsibility.")

    st.markdown('<span class="wf-actions"></span>', unsafe_allow_html=True)
    back_col, _, proceed_col = st.columns([1.1, 5.4, 3.5])
    if back_col.button("← Back", key=f"wf_output_back_{project_id}", use_container_width=True):
        st.session_state[step_key] = 4
        st.rerun()
    if proceed_col.button("Proceed to Responsibility Tab  ›", key=f"wf_to_responsibility_{project_id}", type="primary", use_container_width=True):
        saved["output_checklist"] = checklist_values
        saved["output_complete"] = all(checklist_values.values())
        saved["output_saved"] = True
        saved["saved_by"] = user["email"]
        ...
        saved["active_step"] = 5
        save_workflow(project_id, saved)
        st.switch_page("pages/11_PE_Responsibility.py")
