...
def load_workflow(problem_id):
    """Return the saved operational workflow for one project."""
    conn = get_conn()
    row = conn.execute(
        "SELECT workflow_json FROM project_execution WHERE problem_id=?",
        (problem_id,),
    ).fetchone()
    conn.close()
    if not row or not row["workflow_json"]:
        return {}
    try:
        value = json.loads(row["workflow_json"])
        return value if isinstance(value, dict) else {}
    except (TypeError, json.JSONDecodeError):
        return {}


def save_workflow(problem_id, workflow):
    """Persist the complete operational workflow without overwriting other stages."""
    conn = get_conn()
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    conn.execute(
        """
        INSERT INTO project_execution(problem_id, workflow_json, created_at, updated_at)
        VALUES (?, ?, ?, ?)
        ON CONFLICT(problem_id) DO UPDATE SET
            workflow_json=excluded.workflow_json,
            updated_at=excluded.updated_at
        """,
        (problem_id, json.dumps(workflow), now, now),
    )
    conn.commit()
    conn.close()
...
# ==========================================================
# GOVERNANCE OPERATING MODEL & DECISION RIGHTS
# ==========================================================

def load_responsibility(problem_id):
    """Return the saved governance operating model for one project."""
    conn = get_conn()
    row = conn.execute(
        "SELECT responsible_json FROM project_execution WHERE problem_id=?",
        (problem_id,),
    ).fetchone()
    conn.close()
    if not row or not row["responsible_json"]:
        return {}
    try:
        value = json.loads(row["responsible_json"])
        return value if isinstance(value, dict) else {}
    except (TypeError, json.JSONDecodeError):
        return {}


def save_responsibility(problem_id, responsibility):
    """Persist the complete governance operating model without losing other delivery data."""
    conn = get_conn()
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    conn.execute(
        """
        INSERT INTO project_execution(problem_id, responsible_json, created_at, updated_at)
        VALUES (?, ?, ?, ?)
        ON CONFLICT(problem_id) DO UPDATE SET
            responsible_json=excluded.responsible_json,
            updated_at=excluded.updated_at
        """,
        (problem_id, json.dumps(responsibility), now, now),
    )
    conn.commit()
    conn.close()
...
def load_delivery_gate(problem_id):
    conn = get_conn()
    row = conn.execute(
        "SELECT delivery_gate_json FROM project_execution WHERE problem_id=?",
        (problem_id,),
    ).fetchone()
    conn.close()
    if not row or not row["delivery_gate_json"]:
        return {}
    try:
        value = json.loads(row["delivery_gate_json"])
        return value if isinstance(value, dict) else {}
    except (TypeError, json.JSONDecodeError):
        return {}


def save_delivery_gate(problem_id, gate):
    conn = get_conn()
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    conn.execute(
        """
        INSERT INTO project_execution(problem_id, delivery_gate_json, created_at, updated_at)
        VALUES (?, ?, ?, ?)
        ON CONFLICT(problem_id) DO UPDATE SET
            delivery_gate_json=excluded.delivery_gate_json,
            updated_at=excluded.updated_at
        """,
        (problem_id, json.dumps(gate), now, now),
    )
    conn.commit()
    conn.close()
